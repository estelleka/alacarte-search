...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const pinSVG = () => `<svg width="26" height="34" viewBox="0 0 28 36" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M14 0C6.27 0 0 6.27 0 14c0 10.5 14 22 14 22s14-11.5 14-22C28 6.27 21.73 0 14 0z" fill="#DA3743"/>
  ...
  function renderHeroBanner() {
    ...
    var bgColor = isFallback          ? '#DA3743'
                : (hasGeo && !cuisines.length) ? '#1A1A1A'
                : cuisines.length     ? '#BA2D38'
                : hasGeo              ? '#1A1A1A'
                : '#DA3743';
  ...
  function openModal(hit) {
    ...
    if (hit._geoloc && hit._geoloc.lat && hit._geoloc.lng) {
      ...
      requestAnimationFrame(function () {
        ...
        const pinIcon = L.divIcon({ html: '<svg width="26" height="34" viewBox="0 0 28 36" fill="none"><path d="M14 0C6.27 0 0 6.27 0 14c0 10.5 14 22 14 22s14-11.5 14-22C28 6.27 21.73 0 14 0z" fill="#DA3743"/><circle cx="14" cy="14" r="5" fill="#FFFFFF"/></svg>', className: '', iconSize: [26, 34], iconAnchor: [13, 34] });
